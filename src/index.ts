export const basicAuth = (username: string, password: string): string => 'Basic ' + Buffer.from(`${username}:${password}`, 'binary').toString('base64');
...
export const decodeBase64 = (base64encodedStr: string): string => (Buffer.from(base64encodedStr, 'base64')).toString('utf-8');

export const encodeToBase64 = (str: string): string => (Buffer.from(str, 'utf-8')).toString('base64');
 ...
 */
export const localizedWorldDatetime = (timeZone = 'America/Chicago'): string => (new Date()).toLocaleString('en-US', { timeZone });
...
export const pad = (num: number, charLength = 2, charPadding = '0'): string => num.toString().padStart(charLength, charPadding);